...
void BattlegroundQueue::FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id)
{
    ...
    if (!bg->isArena())
        for (uint8 itr1 = 0; itr1 != 2; ++itr1)
            for (GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + itr1].begin(); itr != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + itr1].end(); ++itr)
                m_SelectionPools[TEAM_ALLIANCE + itr1].AddGroup((*itr), (*itr)->Players.size());
...
bool BattlegroundQueue::CheckCrossFactionMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam)
{
    ...
    {
        //if selection pool is full then break;
        if (!(*itr_team2)->IsInvitedToBGInstanceGUID && !m_SelectionPools[otherTeam].AddGroup(*itr_team2, MaxPlayersPerTeam))
            break;
    }
    if (m_SelectionPools[otherTeam].GetPlayerCount() < MinPlayersPerTeam)
        return false;
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    if (!isRated)
    {
        ...
        if (CheckNormalMatch(bg_template, bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam)
            ...
            || (!bg_template->isArena() && CheckCrossFactionMatch(bracket_id,MinPlayersPerTeam,MaxPlayersPerTeam)))
        {
