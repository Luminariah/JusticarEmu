...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    m_playerLoading = false;
        ...
        if (z < pCurrChar->GetMap()->GetHeight(x, y, MAX_HEIGHT) && pCurrChar->GetMap()->IsOutdoors(x, y, z))
        {
            uint32 m = pCurrChar->m_homebindMapId;
            float  x = pCurrChar->m_homebindX;
            float  y = pCurrChar->m_homebindY;
            float  z = pCurrChar->m_homebindZ;
            float  o = pCurrChar->GetOrientation();
            pCurrChar->TeleportTo(m,x,y,z,o);
        }
