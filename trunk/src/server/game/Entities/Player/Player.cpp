...
void Player::Update(uint32 p_time)
{
    ...
    if (m_ClearFakePlayerlist)
    {
        if (!IsInMap(this))
            m_ClearFakePlayerlist_timer = 0;
        else
        {
            m_ClearFakePlayerlist_timer += p_time;
            if (m_ClearFakePlayerlist_timer >= IN_MILLISECONDS)
            {
                m_ClearFakePlayerlist_timer = 0;
    ...
    if (m_ForgetPlayersInBG)
    {
        if (!IsInMap(this))
            m_ForgetPlayersInBG_timer = 0;
        else
        {
            m_ForgetPlayersInBG_timer += p_time;
            if (m_ForgetPlayersInBG_timer >= IN_MILLISECONDS)
            {
                m_ForgetPlayersInBG_timer = 0;
...
void Player::SetPlayerInTeam(Battleground* bg, uint32 instanceId, BattlegroundTypeId bgTypeId, bool loggedIn)
{
    if (bg->isArena())
        return;
    if (!loggedIn)
    {
        uint32 GrpTeam = TEAM_OTHER;
        uint8 GrpSize = 0;
        if (Group *pGroup = GetGroup())
        {
            GrpSize = GetGroup()->GetMembersCount();
            for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                Player* pGroupGuy = itr->getSource();
                if (!pGroupGuy || pGroupGuy == this)
                    continue;

                if (pGroupGuy->GetBattleground() && pGroupGuy->GetBattleground()->GetInstanceID() == instanceId)
                {
        ...
        /*if (GrpTeam != TEAM_OTHER && bg->GetPlayersCountByTeam(GrpTeam) < bg->GetMaxPlayersPerTeam()) // DISCARD GROUPS
            SetBGTeam(GrpTeam);*/
        if (GrpTeam != TEAM_OTHER && bg->GetPlayersCountByTeam(GrpTeam) + GrpSize - 1 <= bg->GetMaxPlayersPerTeam()) // KEEP GROUPS
            SetBGTeam(GrpTeam);
        else
        {
    ...
    ChatHandler(GetSession()).PSendSysMessage("%s[Battleground system]%s You will be playing for the %s in this battleground",MSG_COLOR_MAGENTA,MSG_COLOR_WHITE,GetBGTeam() == ALLIANCE ? "Alliance" : "Horde");
    bg->UpdatePlayersCountByTeam(GetBGTeam(), false); // Add here instead of in AddPlayer, because AddPlayer is not made until loading screen is finished. Which can cause unbalance in the system.
}

void Player::FitPlayerInTeam(bool action)
{
    if (GetTeam() != GetBGTeam() && action)
    {
        if (GetBattleground() && GetBattleground()->isArena())
            return;
        switch (urand(1,2))
        {
        case 1:
