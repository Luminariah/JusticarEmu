...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    switch (type)
    {
		...
		case CHAT_MSG_OFFICER: 
        {
			...
			{
			    case SEC_PLAYER: // Player (non-vip) Alliance = Blue, Horde = Red
					if (GetPlayer()->GetTeam()==ALLIANCE) {
						snprintf(message, 1024, "|cff05B8CC[Alliance]|cff05B8CC[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					}
				...
				case SEC_VIP: // 1, VIP (Black)
					snprintf(message, 1024, "|cff000000[VIP]|cff000000[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					break;

			    case SEC_MODERATOR: // 2, Trial GM (Gray)
					if (GetPlayer()->isGameMaster()==TRUE)
					{
					snprintf(message, 1024, "|cffA1A1A1[Trial GM]|cffA1A1A1[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					}
					{
			    ...
			    case SEC_GAMEMASTER: // 3, GM (Green)
					if (GetPlayer()->isGameMaster()==TRUE)
					{
					snprintf(message, 1024, "|cff4CBB17[GM]|cff4CBB17[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					}
					{
					if (GetPlayer()->isGameMaster()==FALSE)
					if (GetPlayer()->GetTeam()==HORDE)
					...
					if (GetPlayer()->isGameMaster()==FALSE)
					if (GetPlayer()->GetTeam()==ALLIANCE)
					snprintf(message, 1024, "|cff05B8CC[Alliance]|cff05B8CC[%s]:|cffffffff %s", GetPlayer()->GetName().c_str());
					}
					break;
					
				case SEC_DEVELOPER: // 4, DEVELOPER (Red)
					if (GetPlayer()->isGameMaster()==TRUE)
					{
					snprintf(message, 1024, "|cffFF3030[Developer]|cffFF3030[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					}
					{
					if (GetPlayer()->isGameMaster()==FALSE)
					if (GetPlayer()->GetTeam()==HORDE)
			    ...
			    case SEC_ADMINISTRATOR: // 5, ADMIN (Violet)
					if (GetPlayer()->isGameMaster()==TRUE)
					{
					snprintf(message, 1024, "|cffBF5FFF[Administrator]|cffBF5FFF[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					}
					{
					if (GetPlayer()->isGameMaster()==FALSE)
					if (GetPlayer()->GetTeam()==HORDE)
				...
				case SEC_CONSOLE: // 6, ADMIN (Blue/Orange)
					if (GetPlayer()->isGameMaster()==TRUE)
					{
					snprintf(message, 1024, "|cff00B2EE[Owner]|cfff49956[%s]:|cffffffff %s", GetPlayer()->GetName().c_str(), msg.c_str());
					}
					{
					if (GetPlayer()->isGameMaster()==FALSE)
					if (GetPlayer()->GetTeam()==HORDE)
